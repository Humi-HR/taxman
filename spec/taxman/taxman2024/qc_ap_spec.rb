...
RSpec.describe Taxman2024::QcAp do
  ...
  context "when the employee has already reached the qpip maximum for the year" do
    let(:qc_a6) { Taxman2024::Qpip::EMPLOYEE_MAX }
    let(:qc_s4) { 5_000_00 }
  ...
  context "when the employee has reached qpip maximum for the year less $25" do
    let(:qc_a6) { Taxman2024::Qpip::EMPLOYEE_MAX - 25_00 }
    let(:qc_s4) { 5_000_00 }

    it "calculates zero qpip contribution for this period" do
      expect(c).to eq 24_70
    end
  end

  context "when the employee has reached qpip maximum for the year less $100 and low income" do
    let(:qc_a6) { Taxman2024::Qpip::EMPLOYEE_MAX - 100_00 }
    let(:qc_s4) { 50_00 }
